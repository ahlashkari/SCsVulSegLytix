...
This implementation is used during training since it's more flexible
and is compatible with torch.compile.
...
class BertPooler(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        return first_token_tensor#pooled_output
...
class BertForSequenceClassification(BertPreTrainedModel):
    def __init__(self, config, mlp_head=False):
        super().__init__(config)
        ...
        self.classifier = (nn.Linear(config.hidden_size, config.num_labels) if not mlp_head
                           else nn.Sequential(nn.Linear(config.hidden_size, 64),
                                              nn.GELU(),
